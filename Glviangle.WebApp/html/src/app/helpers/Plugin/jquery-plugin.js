...
(function ($) {
  ...
  jQuery.fn.wait = function (duration, completeCallback) {
    ...
    logger.log(`completeCallback`, completeCallback);
  ...
  jQuery.fn.deSerializeObject = function (obj, by = "name") {
    Object.keys(obj).forEach((key) => {
      const value = obj[key];
      const el = this.find(`[name="${key}"]`);
      if (el.length) {
        if (
          ["checkbox", "radio"].includes(el.attr("type")) &&
          el[0].value === value
        ) {
          el[0].checked = true;
